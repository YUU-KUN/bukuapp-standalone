...
export default {
  ...
  created() {
    this.axios.get(`http://localhost:8000/api/buku/${this.$route.params.id}`).then((res) => {
      this.buku = res.data;
    });
  },
  methods : {
    edit() {
      this.axios.put(`http://localhost:8000/api/buku/${this.$route.params.id}`, this.buku).then(response => (
